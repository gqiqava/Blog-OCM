  ...
  <div class="section container" style="height: 100%;">
    <p style="text-align: left; margin-top: 60px; margin-bottom: 60px;">
       <router-link :to="{ name: 'singleCategory'}">
        ...
        <img src="@/assets/arr.svg" alt="ZuniLife" style="width: 20px; margin-left: 5px;"><a style=" margin-left: 5px;" v-bind:class="{ clWhite: dark == false,  clDark: dark == true }">{{ $t('LanguageEn.termsC') }} </a>
    </p>      
  <h1 style="text-align: left; font-size: 24px;">{{text.title}}</h1>
  <p style="text-align: left; font-size: 15px;" v-html="text.text">{{text.text}}</p>
  </div>
</template>
...
this.cookiesObject = cookiesObject;
        ...
        fetch(window.API+"/pageInfo?languageId=" + 2 + "&pageId=" + 2, requestOptions)
